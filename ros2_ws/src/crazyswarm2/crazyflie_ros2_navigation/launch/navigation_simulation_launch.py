...
def generate_launch_description():
    ...
    # Specify the actions
    SM_SD = get_package_share_directory("crazyflie_ros2_simple_mapper")
    print(os.path.join(SM_SD, 'map.yaml'),)

    bringup_cmd = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(launch_dir, 'bringup_launch.py')),
    launch_arguments={ 'slam': 'False',
                       'map': os.path.join(SM_SD, 'map.yaml'),
                       'use_sim_time': 'true',
    ...
    simulation_node = IncludeLaunchDescription(
            PythonLaunchDescriptionSource(
                os.path.join(SM_SD, 'launch', 'simple_mapper_simulation_launch.py')
            )
        )
